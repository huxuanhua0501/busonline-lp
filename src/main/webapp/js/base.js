	...
	$.each(obj.fieldModel, function(index, value){
		...
		if(value.type!="checkbox"&&value.type!="radio"){
			...
			if(value.type == 'text') {
				...
				condition.append($.field.typeahead(value,num,obj.x,obj.y));//.appendTo(row)
				row.append(condition);
			}
...
$.field = {
	...
	selected: function(f,n,x,y) {
		...
		if(f.all==false){
			...
			selected.append($("<option value=''>全部</option>"))
		}
	...
	button : function(buttons,more,n){
		n=n==undefined?2:n;//防止n未定义 
		var d = $("<div class='col-md-"+ n +" pull-right no-padding'></div>");
	...
	typeahead: function(f,n,x,y) {
		var f_text =  "<div class='col-md-"+ n +" '>"  
						+ "<label class='col-sm-"+ x +" control-label' for='form-field-1'> " + f.label + " </label>"
				       	+ "<input type='text' id='" + f.id + "' class='col-md-"+ y +" typeahead scrollable' name='" + f.name + "'/>"
				      + "</div>";
		return $(f_text);
	}
...
function getNoMsgImage(){
	var root= $("#ctx").val();
	return root+"/resource-v1/images/warning.png";
}
